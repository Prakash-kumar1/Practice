...
// Answer => Non-Blocking methods are executed asynchronously. Asynchronously means that the program may not necessarily execute line by line. The program calls the function and move to the next operation and does not wait for it to return.


// Question no-2 => What are the differences between 
// Answer => nonblocking and asynchronous IO system calls?
...
// Answer=> All of the I/O methods in the Node. js standard library provide asynchronous versions, which are non-blocking, and accept callback functions. Some methods also have blocking counterparts, which have names that end with Sync .


// Question no-4=>how does javascript handle asynchronous behaviour??
// Answer=> With asynchronous programming, JavaScript programs can start long-running tasks, and continue running other tasks in parallell. But, asynchronus programmes are difficult to write and difficult to debug. Because of this, most modern asynchronous JavaScript methods don't use callbacks.
...
// Answer => class based components are made up of multiple functions that add functionality to the application. All class based components are child classes for the Component class of ReactJS.


// Question no- 13=> difference between map and foreach ??
// Answer=> The map() method is used to transform the elements of an array, whereas the forEach() method is used to loop through the elements of an array. The map() method can be used with other array methods, such as the filter() method, whereas the forEach() method cannot be used with other array methods.


// Question no-14=> What is Event loop ??
// Answer => An event loop is something that pulls stuff out of the queue and places it onto the function execution stack whenever the function stack becomes empty.


// Question no-15=> What is  this keyword ??
// Answer => In regular function,  this keyword  represents global object that called the function which could be window, the document, a button or whatever. with arrow functions  this keyword  always represents the object that define arrow functions. 
...
// Answer => Currying is a process in functional programming  in which we can transform a function with multiple arguments into a sequence of nesting function . It returns a new function that excepts the next argument inline .
...
// Answer => Debouncing is a programming pattern or a technique to restrict the calling of a time-consuming function frequently, by delaying the execution of the function until a specified time to avoid unnecessary CPU cycles, and API calls and improve performance.
